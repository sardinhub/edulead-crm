...
import {
  ...
  Trash2, Database, Download, FileText, FileSpreadsheet, Eye, X
} from 'lucide-react';
                    ...
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {r.responded_leads_details && r.responded_leads_details.length > 0 && (
                            <button 
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 text-lg">Rincian Leads Responsif</h3>
                    <p className="text-xs text-slate-500">Laporan: {new Date(selectedReportDetails.report_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long' })} oleh {selectedReportDetails.staff_name}</p>
                  </div>
                </div>
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {selectedReportDetails.responded_leads_details.map((lead, idx) => (
                  <div key={idx} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-colors group">
                    <div className="flex justify-between items-start mb-2">
                       <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Siswa #{idx + 1}</span>
                    </div>
                    <h4 className="font-bold text-slate-900 text-base">{lead.name || 'Tanpa Nama'}</h4>
                    <div className="mt-2 grid grid-cols-2 gap-4 text-xs">
                      <div>
                        <p className="text-slate-400 font-medium mb-0.5">WhatsApp / HP</p>
                        <p className="text-slate-700 font-bold">{lead.phone || '—'}</p>
                      </div>
                      <div>
                        <p className="text-slate-400 font-medium mb-0.5">Asal Sekolah</p>
                        <p className="text-slate-700 font-bold">{lead.school || '—'}</p>
                      </div>
                    </div>
                    {lead.note && (
                      <div className="mt-3 p-2 bg-white rounded-lg border border-slate-100 text-xs text-slate-600 leading-relaxed italic">
                        "{lead.note}"
                      </div>
                    )}
                  </div>
