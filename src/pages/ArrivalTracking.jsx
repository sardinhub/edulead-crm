                ...
                <tr key={item.id} className="hover:bg-indigo-50/30 transition-all group">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex justify-center">
                      <select
                        value={item.arrival_status || 'BELUM KONFIRMASI'}
                        onChange={(e) => updateLeadArrivalStatus(item.id, e.target.value)}
                        className="w-full max-w-[180px] px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-[11px] font-bold text-slate-700 cursor-pointer text-center"
                      >
                        {ARRIVAL_STATUSES.map(st => (
                          <option key={st.id} value={st.id}>{st.label}</option>
                        ))}
