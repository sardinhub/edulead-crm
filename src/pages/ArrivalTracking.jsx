...
export default function ArrivalTracking() {
  const { students, updateArrivalStatus, user } = useStore();
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');

  // Filter: Hanya siswa yang sudah PANGKAL LUNAS
  const paidStudents = useMemo(() => {
    return students.filter(s => {
      const isPaid = s.status_pembayaran?.includes('Pangkal Full') || 
                     s.status_pembayaran?.includes('Pangkal Lunas') ||
                     s.status_pembayaran?.includes('WON');
      ...
      const matchesArrivalStatus = filterStatus === 'all' || (s.arrival_status || 'BELUM KONFIRMASI') === filterStatus;

      return isPaid && matchesSearch && matchesArrivalStatus;
    });
  ...
  const stats = useMemo(() => {
    ...
    paidStudents.forEach(s => {
      if (counts[s.arrival_status]) counts[s.arrival_status]++;
    });
    return counts;
  }, [paidStudents]);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 uppercase text-[10px] font-bold tracking-widest">
            Khusus Siswa Status PANGKAL LUNAS
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        {ARRIVAL_STATUSES.filter(st => st.id !== 'BELUM KONFIRMASI').map(st => (
          <div key={st.id} className={cn("p-4 rounded-3xl border shadow-sm transition-all", st.bg, st.ring.replace('ring-', 'border-'))}>
            ...
            <p className={cn("text-2xl font-black", st.text)}>{paidStudents.filter(s => s.arrival_status === st.id).length}</p>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 font-black text-slate-400 text-[10px] uppercase tracking-wider text-center">Status Pembayaran</th>
              ...
              {paidStudents.map((item) => (
                <tr key={item.id} className="hover:bg-indigo-50/30 transition-all group">
                  ...
                  <td className="px-6 py-5 text-center">
                    <span className="px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-tighter bg-emerald-50 text-emerald-600 ring-1 ring-emerald-100 shadow-sm">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2">
                      {ARRIVAL_STATUSES.map(st => {
                        ...
                        return (
                          <button
                            key={st.id}
                            onClick={() => updateArrivalStatus(item.id, st.id)}
                            className={cn(
              ...
              {paidStudents.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-20 text-center text-slate-300 italic font-medium">
                    Tidak ada siswa lunas yang sesuai dengan filter pencarian.
                  </td>
                </tr>
              )}
            </tbody>
